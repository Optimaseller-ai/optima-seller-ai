...
export type Prospect = {
  ...
  created_at: string;
  updated_at: string;
  interestLevel?: "cold" | "warm" | "hot" | "ready";
  tags?: string[];
  salesScore?: number;
};
          ...
          <div className="flex items-center gap-2">
            ...
            {prospect.interestLevel && (
              <div className={cn(
                ...
                prospect.interestLevel === "hot" && "bg-red-500",
                prospect.interestLevel === "warm" && "bg-orange-500",
                prospect.interestLevel === "cold" && "bg-blue-500",
                prospect.interestLevel === "ready" && "bg-green-500",
              )}
              title={`Température: ${prospect.interestLevel}`}
            />
            )}
          </div>
          {prospect.phone ? (
      ...
      {prospect.tags && prospect.tags.length > 0 && (
        <div className="mt-2 flex flex-wrap gap-1">
          {prospect.tags.map(tag => (
            <span key={tag} className="rounded-full bg-secondary px-1.5 py-0.5 text-[10px] font-medium text-secondary-foreground uppercase tracking-tight">
              {tag}
            </span>
          ))}
        </div>
