...
type ChatHeaderProps = {
  ...
  onBack?: () => void;
  onRequestClearConversation?: () => void;
  ...
  return (
    ...
    >
      <div className="mx-auto grid w-full max-w-[920px] grid-cols-[1fr_auto] items-center gap-2 px-3 py-2 min-[1200px]:max-w-none min-[1200px]:grid-cols-[1fr_auto_1fr] min-[1200px]:gap-3 min-[1200px]:px-6 min-[1400px]:px-8">
        ...
        <div className="relative ml-auto flex items-center justify-end gap-0.5 min-[1200px]:gap-1" ref={menuRef}>
          <div
          ...
          <button
            className={headerIconBtn(props.darkMode)}
            aria-label="Options"
            type="button"
          ...
          {menuOpen ? (
            <div
              className={`absolute right-0 top-full z-50 mt-1 min-w-[220px] rounded-xl border py-1 shadow-lg ${
                props.darkMode ? "border-white/10 bg-[#151b24]" : "border-slate-200 bg-white"
              }`}
            >
              <button
                type="button"
                className={`block w-full px-3 py-2 text-left text-[13px] ${props.darkMode ? "text-slate-200 hover:bg-white/5" : "text-slate-800 hover:bg-slate-50"}`}
                onClick={() => {
                  setMenuOpen(false);
                  props.onRequestClearConversation?.();
                }}
              >
