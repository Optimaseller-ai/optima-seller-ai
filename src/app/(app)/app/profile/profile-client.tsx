  ...
  async function saveNow(opts?: { showSuccessToast?: boolean }) {
    ...
    try {
      ...
      if (opts?.showSuccessToast !== false) {
        toast({ title: "EnregistrÃ©", description: "Profil synchronisÃ© avec Supabase." });
      }
      setDirty(false);
    } catch (err: unknown) {
      toast({
    ...
    <div className="space-y-6 sm:space-y-8">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm text-[var(--brand-navy)]/65 sm:text-base">Ces infos alimentent votre IA (gÃ©nÃ©rateur + assistant).</p>
        </div>
      ...
      <section className="grid gap-3 lg:grid-cols-3">
        <Card className="border-[var(--brand-navy)]/10 bg-white shadow-[0_12px_35px_rgba(15,23,42,0.06)] lg:col-span-2">
          <CardHeader>
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <CardDescription>Plus le profil est complet, plus lâ€™IA est pertinente.</CardDescription>
              </div>
          ...
          <CardContent className="grid gap-4 p-4 sm:grid-cols-2 sm:p-6">
            ...
            <Field icon={Building2} required label="Nom de la sociÃ©tÃ©" value={form.business_name} onChange={(v) => setField("business_name", v)} placeholder="Ex: Nadine Beauty" />
            ...
            <Field
              ...
              helper="UtilisÃ© pour support et compte. Si vide, on prend lâ€™email du compte."
            />
        ...
        <div className="space-y-3">
          <Card className="border-[var(--brand-navy)]/10 bg-white shadow-[0_12px_35px_rgba(15,23,42,0.06)]">
            <CardHeader>
              ...
              <CardDescription>DonnÃ©es liÃ©es Ã  lâ€™auth Supabase.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3 p-4 sm:p-6">
              <AccountRow label="Email connectÃ©" value={accountEmail ?? "â€”"} />
              <AccountRow label="Membre depuis" value={signedUpAt ? new Date(signedUpAt).toLocaleDateString("fr-FR") : "â€”"} icon={CheckCircle2} />
              <AccountRow
                label="DerniÃ¨re sauvegarde"
                value={lastSyncedAt ? new Date(lastSyncedAt).toLocaleString("fr-FR") : "â€”"}
                icon={CheckCircle2}
              />
          ...
          <Card className="border-[rgba(245,158,11,0.25)] bg-[linear-gradient(135deg,rgba(245,158,11,0.12),rgba(22,163,74,0.08))] shadow-[0_18px_55px_rgba(15,23,42,0.10)]">
            <CardHeader>
              <CardTitle className="text-[var(--brand-navy)]">MÃ©moire IA</CardTitle>
              <CardDescription>Activez un contexte business stable pour des rÃ©ponses plus â€œclosingâ€.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3 p-4 sm:p-6">
              <div className="rounded-2xl border border-[var(--brand-navy)]/10 bg-white/80 p-3 text-sm text-[var(--brand-navy)]/75">
                <span className="font-semibold text-[var(--brand-navy)]">Conseil :</span> dÃ©crivez votre offre, votre prix et votre zone de livraison.
              </div>
              <Button asChild className="w-full">
                <Link href="/app/chat">Tester lâ€™assistant IA</Link>
              </Button>
            </CardContent>
          </Card>
        </div>
