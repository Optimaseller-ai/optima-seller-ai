        ...
        >
          <div className="flex items-end gap-1">
            ...
            <button
              ...
              disabled={sending}
              className={`${iconHitClass(darkMode)} ${recordingVoice ? (darkMode ? "bg-red-500/20 text-red-300" : "bg-red-500/10 text-red-600") : ""}`}
              aria-label={recordingVoice ? "Arrêter l'enregistrement" : "Note vocale"}
            >
              <Mic className={`h-[15px] w-[15px] ${recordingVoice ? "animate-pulse" : ""}`} strokeWidth={1.75} />
            </button>
            <motion.button
              type="button"
              whileTap={{ scale: 0.96 }}
