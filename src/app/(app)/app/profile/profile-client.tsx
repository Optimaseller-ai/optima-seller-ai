...
import { BusinessKnowledgeSection } from "./business-knowledge-section";
...
export function ProfileClient(props: {
  knowledgeIdentity: ProfileIdentityForKnowledge;
  knowledgeSettings: BusinessKnowledgeSettingsRow | null;
  knowledgeFaq: BusinessFaqEntry[];
}) {
    ...
    <div className="space-y-6 sm:space-y-8">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Badge variant={mem.status === "active" ? "success" : "gold"} title="Mémoire business pour l'IA">
            <MessageCircleMore className="size-3.5 text-[var(--brand-green)]" />
            Mémoire {mem.status === "active" ? "active" : "à configurer"}
          </Badge>
        ...
        <Card className="border-[var(--brand-navy)]/10 bg-white shadow-[0_12px_35px_rgba(15,23,42,0.06)] lg:col-span-2">
          ...
          <CardContent className="grid gap-4 p-4 sm:grid-cols-2 sm:p-6">
            ...
            <Field
              ...
              placeholder="+237…"
            />
            <Field label="Offre (produit/service + prix si possible)" value={form.offer} onChange={(v) => setField("offer", v)} placeholder="Ex: Packs skincare, livraison 24–48 h…" />
            <Field
              label="Email"
              value={form.email || accountEmail || ""}
              onChange={(v) => setField("email", v)}
              placeholder="email@exemple.com"
              helper="Utilisé pour le support et le compte. Si vide, on prend l'email du compte."
            />
        ...
        <div className="space-y-3">
          <Card className="border-[var(--brand-navy)]/10 bg-white shadow-[0_12px_35px_rgba(15,23,42,0.06)]">
            <CardHeader>
              ...
              <CardDescription>Données liées à l'authentification Supabase.</CardDescription>
            </CardHeader>
          ...
          <Card className="border-[rgba(245,158,11,0.25)] bg-[linear-gradient(135deg,rgba(245,158,11,0.12),rgba(22,163,74,0.08))] shadow-[0_18px_55px_rgba(15,23,42,0.10)]">
            <CardHeader>
              ...
              <CardDescription>Contexte business pour des réponses naturelles et fiables.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3 p-4 sm:p-6">
              <div className="rounded-2xl border border-[var(--brand-navy)]/10 bg-white/80 p-3 text-sm text-[var(--brand-navy)]/75">
                <span className="font-semibold text-[var(--brand-navy)]">Conseil :</span> complétez le profil puis la base de connaissance ci-dessous.
              </div>
      ...
      <BusinessKnowledgeSection
        identity={{
          ...props.knowledgeIdentity,
          businessName: form.business_name.trim() || props.knowledgeIdentity.businessName,
          sector: form.business_type.trim() || props.knowledgeIdentity.sector,
          country: form.country.trim() || props.knowledgeIdentity.country,
          city: form.city.trim() || props.knowledgeIdentity.city,
          offer: form.offer.trim() || props.knowledgeIdentity.offer,
          goal: form.goal.trim() || props.knowledgeIdentity.goal,
          contactPhone: form.contact_phone.trim() || props.knowledgeIdentity.contactPhone,
        }}
        settings={props.knowledgeSettings}
        faqEntries={props.knowledgeFaq}
      />
