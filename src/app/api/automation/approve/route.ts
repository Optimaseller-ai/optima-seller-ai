...
export async function POST(req: Request) {
  ...
  const resume = await resumeWorkflowAfterHumanApproval({ jobId, choice: "send" });
  if (!resume.dispatched && resume.error === "job_not_found_or_not_awaiting") {
  ...
  return NextResponse.json({
    ok: true,
    n8nDispatched: resume.dispatched,
    runId: resume.runId,
