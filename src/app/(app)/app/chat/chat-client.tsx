...
export function ChatClient() {
  ...
  const profileCompletion = React.useMemo(() => {
    ...
    if (!p) return 0;
    const fields = ["business_name", "business_type", "city", "offer", "goal"];
    const filled = fields.filter((f) => p[f as keyof typeof p]?.trim()).length;
  ...
  async function send(userText: string) {
    ...
    try {
      ...
      if ((res as any).insights) setInsights((res as any).insights);
    ...
    <div className="space-y-6 sm:space-y-8">
                      ...
                      <div className="inline-flex items-center gap-2 text-xs text-[var(--brand-navy)]/65">
                        ...
                        {!quota.loading ? (
                          <div className="flex items-center gap-2">
                            <div className="h-2 w-24 overflow-hidden rounded-full bg-[var(--brand-navy)]/10">
                              <div
                                className={cn(
                                  "h-full transition-all duration-500",
                                  quota.used / quota.limit > 0.8 ? "bg-orange-500" : "bg-[var(--brand-green)]"
                                )}
        ...
        <div className="space-y-4">
          <Card className="border-[var(--brand-navy)]/10 bg-white shadow-[0_12px_35px_rgba(15,23,42,0.06)]">
            ...
            <CardContent className="space-y-3 p-3 sm:p-6">
              <div className="rounded-2xl border border-[var(--brand-navy)]/10 bg-[hsl(var(--background))] p-4">
                <div className="flex items-center justify-between text-sm font-semibold text-[var(--brand-navy)]">
                  <span>Complétude profil</span>
                  <span className="text-xs font-bold text-[var(--brand-green)]">{profileCompletion}%</span>
                </div>
                <div className="mt-2 h-2 w-full overflow-hidden rounded-full bg-[var(--brand-navy)]/10">
                  <div
                    className={cn(
                      "h-full transition-all duration-500",
                      profileCompletion === 100 ? "bg-[var(--brand-green)]" : "bg-[var(--brand-gold)]"
                    )}
                  ...
                  <div className="mt-3">
                    <Button asChild className="w-full text-xs h-8">
                      <Link href="/app/profile">
                        Améliorer la mémoire <ChevronRight className="ml-2 size-3" />
                      </Link>
                    </Button>
                  </div>
                )}
      ...
      <Dialog open={isUpsellOpen} onOpenChange={setIsUpsellOpen}>
        <DialogContent className="overflow-hidden p-0">
          <div className="relative bg-[var(--brand-navy)] p-8 text-white">
            <div className="absolute right-0 top-0 opacity-10">
              <Crown className="size-32 text-white" />
            </div>
            <div className="relative space-y-2">
              <div className="inline-flex items-center gap-1.5 rounded-full bg-[var(--brand-gold)] px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-[var(--brand-navy)]">
                <Sparkles className="size-3" />
                Accès Premium
              </div>
              <DialogTitle className="text-2xl font-bold text-white">
                Débloquez la puissance d'Optima Pro
              </DialogTitle>
              <DialogDescription className="text-white/80">
                Ce modèle est réservé aux membres Pro. Accédez à une précision accrue, un style plus persuasif et des quotas étendus.
              </DialogDescription>
            </div>
          </div>
          <div className="p-6 space-y-4">
            <div className="grid gap-3">
              {[
                { label: "Accès à tous les modèles IA", icon: Brain },
                { label: "Réponses premium (3 options)", icon: Sparkles },
                { label: "Quotas de génération augmentés", icon: TrendingUp },
                { label: "Support prioritaire", icon: ShieldCheck },
              ].map((item, i) => (
                <div key={i} className="flex items-center gap-3 text-sm text-[var(--brand-navy)]/70">
            ...
            <DialogFooter className="pt-4">
              <Button asChild className="w-full bg-[var(--brand-gold)] text-[var(--brand-navy)] hover:bg-[var(--brand-gold)]/90 font-bold">
                <Link href="/pricing">
                  Passer au Plan Pro <ChevronRight className="ml-2 size-4" />
                </Link>
              </Button>
            </DialogFooter>
          </div>
