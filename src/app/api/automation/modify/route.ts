...
import { getAutomationJob, resolveAutomationJobHumanApproval } from "@/lib/automation/action-queue";
...
/** Remet la job en file `pending` sans déclencher n8n — pour reprise / édition manuelle ultérieure. */
export async function POST(req: Request) {
  ...
  const updated = resolveAutomationJobHumanApproval({ jobId, choice: "modify" });
  if (!updated) {
    return NextResponse.json({ error: "Action introuvable ou déjà traitée." }, { status: 404 });
  }
